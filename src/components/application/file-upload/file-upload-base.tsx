...
export const FileUploadDropZone = ({
    ...
    return (
        <div
            ...
            onDrop={handleDrop}
            className={cx(
                "relative flex flex-col items-center gap-3 rounded-2xl bg-primary px-4 py-3 text-tertiary ring-1 ring-secondary transition duration-100 ease-linear ring-inset",
                isDraggingOver && "ring-2 ring-brand",
                isDisabled && "cursor-not-allowed bg-secondary",
                className,
            )}
        >
            <FeaturedIcon icon={UploadCloud02} color="gray" theme="modern" size="sm" className={cx(isDisabled && "opacity-50")} />
            ...
            <div className="flex w-full flex-col items-center gap-2 text-center">
                <div className="flex flex-wrap items-center justify-center gap-2">
                    ...
                    <label htmlFor={id} className="flex cursor-pointer">
                        <Button color="link-gray" size="sm" isDisabled={isDisabled} onClick={() => inputRef.current?.click()}>
                            Upload files
                        </Button>
                    </label>
                    <span className="text-sm text-tertiary">or drag & drop</span>
                </div>
                <p className={cx("text-xs leading-5 transition duration-100 ease-linear", isInvalid && "text-error-primary")}>{hint || "PNG, JPEG, or WebP — up to 5MB each."}</p>
            </div>
        </div>
    );
};
...
export const FileListItemProgressBar = ({ name, size, progress, failed, type, fileIconVariant, onDelete, onRetry, className }: FileListItemProps) => {
    ...
    return (
        <motion.li
            layout="position"
            className={cx(
                "relative flex gap-3 rounded-2xl bg-primary p-3 ring-1 ring-secondary transition-shadow duration-100 ease-linear ring-inset",
                failed && "ring-2 ring-error",
                className,
            )}
        >
            <FileTypeIcon className="size-8 shrink-0 dark:hidden" type={type ?? "empty"} theme="light" variant={fileIconVariant ?? "default"} />
            <FileTypeIcon className="size-8 shrink-0 not-dark:hidden" type={type ?? "empty"} theme="dark" variant={fileIconVariant ?? "default"} />
                ...
                <div className="flex w-full items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-tertiary">
                            <span className="truncate whitespace-nowrap">{getReadableFileSize(size)}</span>
                            <span className="hidden h-3 w-px rounded-full bg-border-primary md:inline-block" />
                            <div className="flex items-center gap-1">
                                {isComplete && <CheckCircle className="size-4 stroke-[2.5px] text-fg-success-primary" />}
                                {isComplete && <span className="font-medium text-success-primary">Complete</span>}

                                {!isComplete && !failed && <UploadCloud02 className="size-4 stroke-[2.5px] text-fg-quaternary" />}
                                {!isComplete && !failed && <span className="font-medium text-quaternary">Uploading</span>}

                                {failed && <XCircle className="size-4 text-fg-error-primary" />}
                                {failed && <span className="font-medium text-error-primary">Failed</span>}
                            </div>
                        </div>
                    </div>

                    <ButtonUtility color="tertiary" tooltip="Delete" icon={Trash01} size="xs" className="self-start" onClick={onDelete} />
                </div>

                {!failed && (
                    <div className="w-full">
                        <ProgressBar labelPosition="right" max={100} min={0} value={progress} className="!h-2" />
                    </div>
                )}

                {failed && (
                    <Button color="link-destructive" size="sm" onClick={onRetry} className="mt-1">
                        Try again
                    </Button>
                )}
            </div>
...
export const FileListItemProgressFill = ({ name, size, progress, failed, type, fileIconVariant, onDelete, onRetry, className }: FileListItemProps) => {
    ...
    return (
        <motion.li layout="position" className={cx("relative flex gap-3 overflow-hidden rounded-2xl bg-primary p-3", className)}>
            {/* Progress fill. */}
            <div
            ...
            />
            <FileTypeIcon className="relative size-8 shrink-0 dark:hidden" type={type ?? "empty"} theme="light" variant={fileIconVariant ?? "solid"} />
            <FileTypeIcon className="relative size-8 shrink-0 not-dark:hidden" type={type ?? "empty"} theme="dark" variant={fileIconVariant ?? "solid"} />
            ...
            <div className="relative flex min-w-0 flex-1">
                <div className="relative flex min-w-0 flex-1 flex-col gap-2">
                    <div className="w-full min-w-0 flex-1">
                        ...
                        <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-tertiary">
                            <span>{failed ? "Upload failed, please try again" : getReadableFileSize(size)}</span>
                            {!failed && (
                                <>
                                    <span className="hidden h-3 w-px rounded-full bg-border-primary md:inline-block" />
                                    <div className="flex items-center gap-1">
