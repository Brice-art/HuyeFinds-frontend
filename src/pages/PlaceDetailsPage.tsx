...
export function PlaceDetailsPage() {
  ...
  // until then anyway.
  const { favorited, toggle } = useFavoriteToggle(place?.id ?? "", place?.isFavorited ?? false);

  if (loading) return <p className="px-5 md:px-10 py-10 text-sm text-ink-faint">Loading…</p>;
  if (error || !place) return <p className="px-5 md:px-10 py-10 text-sm text-heart">Couldn't load this place.</p>;
      ...
      <div className="lg:sticky lg:top-6">
        <div className="relative lg:rounded-3xl lg:overflow-hidden">
          ...
          <div className="absolute top-4 right-4 z-10">
            <FavoriteButton favorited={favorited} onToggle={toggle} className="w-10 h-10" />
          </div>
          <img src={cover} alt={place.name} className="w-full aspect-[5/4] lg:aspect-[4/3.3] lg:rounded-3xl object-cover" />
        </div>
      ...
      <div className="lg:min-w-0">
        <div className="px-5 lg:px-0 pt-2 pb-1">
          <p className="text-xs text-ink-faint mb-2">
            {place.category.name} / {place.landmark}
          </p>
        ...
        <div className="flex items-center justify-between bg-surface border border-border rounded-md px-4 py-3.5 mx-5 lg:mx-0 my-4 shadow-soft">
          <div>
            <div className="text-[11px] text-ink-soft mb-0.5">Call to ask about today's menu</div>
            <div className="font-mono text-sm font-semibold">{place.contactPhone}</div>
          </div>
          <a
        ...
        <p className="px-5 lg:px-0 text-sm text-ink-soft leading-relaxed">{place.description}</p>

        {place.menuItems.length > 0 && (
          <>
            <hr className="my-5 mx-5 lg:mx-0 border-border" />
            <h2 className="px-5 lg:px-0 text-lg font-semibold mb-3.5">Menu highlights</h2>
            <div className="px-5 lg:px-0 flex flex-col gap-2.5">
              {place.menuItems.map((item) => (
                <div key={item.id} className="flex items-center justify-between bg-surface border border-border rounded-md px-4 py-3">
                  <div>
                    <div className="text-[13.5px] font-semibold">{item.name}</div>
                    {item.note && <div className="text-[11.5px] text-ink-soft">{item.note}</div>}
                  </div>
                  <PriceTag label={`${item.price.toLocaleString("en-RW")} RWF`} />
                </div>
              ))}
            </div>
          </>
        ...
        {place.hours.length > 0 && (
          <>
            <hr className="my-5 mx-5 lg:mx-0 border-border" />
            <h2 className="px-5 lg:px-0 text-lg font-semibold mb-3.5">Business hours</h2>
            <div className="px-5 lg:px-0">
              {place.hours.map((h) => (
                <div key={h.dayOfWeek} className="flex justify-between py-2.5 border-b border-border last:border-none text-sm">
                  <span className="font-semibold">{DAY_LABELS[h.dayOfWeek]}</span>
        ...
        <hr className="my-5 mx-5 lg:mx-0 border-border" />
        <h2 className="px-5 lg:px-0 text-lg font-semibold mb-3.5">Student reviews</h2>
        {place.reviews.length === 0 && (
          <p className="px-5 lg:px-0 text-sm text-ink-faint mb-2">No reviews yet — be the first to leave one.</p>
        )}
        {place.reviews.map((r) => (
          <div key={r.id} className="bg-surface border border-border rounded-md px-4 py-3.5 mx-5 lg:mx-0 mb-3">
            <div className="flex items-center gap-2.5 mb-2">
              ...
              <div>
                ...
                <div className="text-[11px] text-ink-faint">{new Date(r.createdAt).toLocaleDateString()}</div>
              </div>
            </div>
            {r.comment && <p className="text-[13px] text-ink-soft leading-relaxed">{r.comment}</p>}
          </div>
        ))}
        ...
        {similar && similar.items.length > 0 && (
          <>
            <hr className="my-5 mx-5 lg:mx-0 border-border" />
            <h2 className="px-5 lg:px-0 text-lg font-semibold mb-3.5">Similar places nearby</h2>
