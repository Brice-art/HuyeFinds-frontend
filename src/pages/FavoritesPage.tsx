...
export function FavoritesPage() {
  ...
  useEffect(() => {
    if (!user) return;
    api.get<{ items: PlaceSummary[] }>("/favorites").then((res) => setPlaces(res.items));
  }, [user]);
  ...
  return (
    <div className="px-5 md:px-10 py-6">
      <h1 className="font-display text-2xl font-semibold mb-4">Your favorites</h1>

      {places === null && <p className="text-sm text-ink-faint">Loading…</p>}
      {places?.length === 0 && (
